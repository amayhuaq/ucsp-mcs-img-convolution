...
uchar computeValPix(uchar *image, Filter *filter, int w, int h, int i, int j)
{
	float sumVals = 0;
...
#pragma omp parallel
	{
		...
		if(tid == 0) {
			cout << "Total Threads: " << tN << endl;
			cout << "ID Thread: " << tid << endl;
		}

		for(int i = 0; i < h; i++)
			for(int j = tid; j < w; j += tN)
				out_image[getOffsetPos(w,h,i,j)] = computeValPix(image, filter, w, h, i, j);
	}
}
...
Mat applyFilter(Mat img, Filter *filter)
{
	...
	cout << "Elapsed time: " << ((float)(time)) << " secs\n";
